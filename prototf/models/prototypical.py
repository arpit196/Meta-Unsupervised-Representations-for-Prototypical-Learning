...
class Prototypical(Model):
    ...
    def proto_enc(self, inputs):
        pro_encoder = tf.keras.layers.multiply([self.W[0],self.l1])(inputs)
        pro_encoder = tf.keras.layers.multiply([self.W[1],self.l2])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[2],self.l3])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[3],self.l4])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[4],self.l5])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[5],self.l6])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[6],self.l7])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[7],self.l8])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[8],self.l9])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[9],self.l10])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[10],self.l11])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[11],self.l12])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[12],self.l13])(pro_encoder)
        pro_encoder = tf.keras.layers.multiply([self.W[13],self.l14])(pro_encoder)
        output = tf.keras.layers.multiply([self.W[14],self.l15])(pro_encoder)
